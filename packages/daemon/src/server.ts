...
export class DaemonServer {
  ...
  private state: State;
  private pendingShow: { session: string; timer: NodeJS.Timeout } | undefined;
  ...
  private async onHello(conn: Connection, msg: HelloMessage): Promise<void> {
    ...
    if (msg.focused) this.scheduleShow(sessionName, 0);
  }
  ...
  private scheduleShow(session: string, delayMs: number): void {
  ...
  private enqueueShow(session: string): void {
    this.showQueue = this.showQueue
      .then(() => this.o.presenter.show(session))
      .catch((err) => this.o.log(`presenter.show(${session}) failed: ${(err as Error).message}`));
  }
