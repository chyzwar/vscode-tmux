...
- CLI: `vscode <path[:line[:col]]>` (alias of `vscode-tmux open`), `vscode-tmux new [name] [-- cmd...]`, `vscode-tmux toggle`, `vscode-tmux list`, `vscode-tmux status`.
...
- F11 in the dropdown toggles it between full height and a 45% strip (`~/.config/vscode-tmux/quick-terminal-height.sh`). Ghostty cannot resize or fullscreen a live quick terminal, so the script rewrites `quick-terminal-size`, reloads, and rebuilds the window; the tmux client reconnects in about a second.
