...
- Dropdown size and position: `quick-terminal-*` in `~/.config/vscode-tmux/ghostty.conf` (`quick-terminal-position`, `quick-terminal-size = 45%`, `quick-terminal-autohide` for classic hide-on-focus-loss), then `systemctl --user restart app-dev.vscodetmux.Ghostty.service`.
